...
export const SepiaStickerCommand: Command = {
  name: 'sepiasticker',
  aliases: ['sepia'],
  category: CommandCategory.STICKER,
  description: 'Convert image to sepia sticker',
  usage: 'sepiasticker',
  async execute(ctx) {
    ...
    try {
      ...
      const processedBuffer = await stickerEngine.createSepiaSticker(buffer);
      ...
      await ctx.socket.sendMessage(
        ctx.message.key.remoteJid!,
        { text: 'Failed to create sepia sticker!' },
        { quoted: ctx.message }
      );
    }
  },
...
export default SepiaStickerCommand;
