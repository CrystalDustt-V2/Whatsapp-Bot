...
export const BlackAndWhiteStickerCommand: Command = {
  name: 'bwsticker',
  aliases: ['bw', 'blackwhite', 'blackwhitesticker'],
  category: CommandCategory.STICKER,
  description: 'Convert image to black and white sticker',
  usage: 'bwsticker',
  async execute(ctx) {
    ...
    try {
      ...
      const processedBuffer = await stickerEngine.createBlackAndWhiteSticker(buffer);
      ...
      await ctx.socket.sendMessage(
        ctx.message.key.remoteJid!,
        { text: 'Failed to create black and white sticker!' },
        { quoted: ctx.message }
      );
    }
  },
...
export default BlackAndWhiteStickerCommand;
